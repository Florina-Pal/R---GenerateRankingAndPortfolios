                                    ...
                                    folder, folderMain, type) 
{
  ...
  for(rank in seq(1, length(rankingFilesName),1))
  {
    createIfFolderDontExists(folder[rank])
    #reading ranking data from ranking file
    rankingDataset <- data.frame(read.csv(file =  rankingFilesName[rank], sep=",",header=TRUE,stringsAsFactors=F, fileEncoding="utf-8", check.names=FALSE))
    ...
    for(run_for_years in seq(1,3,1))
    {
      ...
      for(i  in seq(1,length(fromDate),1) )
      {
        ...
        #divide ranking file symbols into bins
        {
          ...
          graterThan80 <- which(rankingData[1,2:dim(rankingData)[2] ]>= 80)
          growth_symbols <- colnames(rankingData)[graterThan80]
          
          between_20_80 <- which(rankingData[1,2:dim(rankingData)[2] ]< 80 & rankingData[1, 2:dim(rankingData)[2]]>20)
          core_symbols <- colnames(rankingData)[between_20_80]
          
          lessThan_20 <- which(rankingData[1, 2:dim(rankingData)[2]]<=20)
          ...
          if( toupper(type) == "ALL"){symbols = c(value_symbols, core_symbols, growth_symbols)}
          if( toupper(type) == "VG"){symbols = c(value_symbols, growth_symbols )}
          
          
        }
        
        portfolioName = paste0(fromDate[i],"_to_", toDate[i],"_",type,"Portfolio_", rankNameForOutputFileNotations[rank])
      ...
      write.csv(average_Summary, file = paste0(folder,"Average_Summary_",type,"_Run_for_",run_for_years, "_years_",rankNameForOutputFileNotations[rank], ".csv"))
      write.csv(final_summary, file = paste0(folder, "Final_Summary_",type,"_Run_for_",run_for_years, "_years_",rankNameForOutputFileNotations[rank], ".csv"))
...
GeneratePortfolioEqualWeighted_LoadFromFile_WithInputSymbols = function(fromDate, toDate, symbolGuidList, timeIndexFileName, fileAppendix)
{
  # ---------------Extract price ------------------------------------------------------------
  ...
  missingSymbolData <- symbolGuidList[!symbolGuidList %in% symbolGuidList_inFile]
  if(length(missingSymbolData)>0)
  {
    print(paste0("These simbols don't have data in timeIndex file ",missingSymbolData))
